...
complete -c ansible-console -l "playbook-dir" -d "Since this tool does not use playbooks, use this as a substitute playbook directory.This sets the relative path for many features including roles/ group_vars/ etc." -r
...
complete -c ansible-console -s "M" -l "module-path" -d "prepend colon-separated path(s) to module library (default=~/.ansible/plugins/modules:/usr/share/ansible/plugins/modules)"
...
complete -c ansible-console -s "v" -l "verbose" -d "verbose mode (-vvv for more, -vvvv to enable connection debugging)"
