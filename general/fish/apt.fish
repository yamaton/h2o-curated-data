...
complete -c apt -n __fish_use_subcommand -x -a list -d 'list packages based on package names'
complete -c apt -n __fish_use_subcommand -x -a search -d 'search in package descriptions'
complete -c apt -n __fish_use_subcommand -x -a show -d 'show package details'
complete -c apt -n __fish_use_subcommand -x -a install -d 'install packages'
complete -c apt -n __fish_use_subcommand -x -a reinstall -d 'reinstall packages'
complete -c apt -n __fish_use_subcommand -x -a remove -d 'remove packages'
complete -c apt -n __fish_use_subcommand -x -a autoremove -d 'Remove automatically all unused packages'
complete -c apt -n __fish_use_subcommand -x -a update -d 'update list of available packages'
complete -c apt -n __fish_use_subcommand -x -a upgrade -d 'upgrade the system by installing/upgrading packages'
complete -c apt -n __fish_use_subcommand -x -a full-upgrade -d 'upgrade the system by removing/installing/upgrading packages'
complete -c apt -n __fish_use_subcommand -x -a edit-sources -d 'edit the source information file'
complete -c apt -n __fish_use_subcommand -x -a satisfy -d 'satisfy dependency strings'
