...
complete -c bat -s l -l language -d 'Explicitly set the language for syntax highlighting.' -x
...
complete -c bat -l file-name -d 'Specify the name to display for a file.' -x
...
complete -c bat -l tabs -d 'Set the tab width to T spaces.' -x
...
complete -c bat -s m -l map-syntax -d 'Map a glob pattern to an existing syntax name.' -x
complete -c bat -l theme -d 'Set the theme for syntax highlighting.' -x
complete -c bat -l list-themes -d 'Display a list of supported themes for syntax highlighting.'
complete -c bat -l style -d 'Configure which elements (line numbers, file headers, grid borders, Git modifications, ..) to display in addition to the file contents.' -x
complete -c bat -s r -l line-range -d 'Only print the specified range of lines for each file.' -x
