...
complete -c bfs -o maxdepth -o mindepth -d 'Ignore files deeper/shallower than N' -x
...
complete -c bfs -o depth -d 'Find files with depth N' -x
...
complete -c bfs -o fstype -d 'Find files on file systems with the given TYPE' -x
complete -c bfs -o gid -o uid -d 'Find files owned by group/user ID N' -x
complete -c bfs -o group -o user -d 'Find files owned by the group/user NAME' -x
complete -c bfs -o ilname -o iname -o ipath -o iregex -o iwholename -d 'Case-insensitive versions of -lname/-name/-path/-regex/-wholename' -x
complete -c bfs -o inum -d 'Find files with inode number N' -x
complete -c bfs -o links -d 'Find files with N hard links' -x
complete -c bfs -o lname -d 'Find symbolic links whose target matches the GLOB' -x
complete -c bfs -o name -d 'Find files whose name matches the GLOB' -x
complete -c bfs -o newer -d 'Find files newer than FILE' -r
complete -c bfs -o newerXY -d 'Find files whose X time is newer than the Y time of REFERENCE.' -x
complete -c bfs -o nogroup -o nouser -d 'Find files owned by nonexistent groups/users'
complete -c bfs -o path -o wholename -d 'Find files whose entire path matches the GLOB' -x
complete -c bfs -o perm -d 'Find files with a matching mode' -x
complete -c bfs -o regex -d 'Find files whose entire path matches the regular expression REGEX' -x
complete -c bfs -o samefile -d 'Find hard links to FILE' -r
complete -c bfs -o size -d 'Find files with the given size, in 1-byte characters, 2-byte words, 512-byte blocks (default), or kiB/MiB/GiB/TiB/PiB' -x
complete -c bfs -o sparse -d 'Find files that occupy fewer disk blocks than expected'
complete -c bfs -o type -d 'Find files of the given type' -x
complete -c bfs -o used -d 'Find files last accessed N days after they were changed' -x
complete -c bfs -o xattr -d 'Find files with extended attributes'
complete -c bfs -o xtype -d 'Find files of the given type, following links when -type would not, and vice versa' -x
...
complete -c bfs -o execdir -o okdir -d 'Like -exec/-ok, but run the command in the same directory as the found file(s)' -x
...
complete -c bfs -o printf -d 'Print according to a format string (see man find).' -x
