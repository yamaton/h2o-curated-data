...
complete -c brew -n "not __fish_seen_subcommand_from install uninstall list search analytics autoremove cask cleanup commands completions config deps desc doctor fetch formulae gist-logs homepage info leaves link log migrate missing options outdated pin pin readall reinstall shellenv tap-info tap unlink unpin untap update-reset update upgrade uses" -l "cache" -d "Display Homebrew\180s download cache." -x
complete -c brew -n "not __fish_seen_subcommand_from install uninstall list search analytics autoremove cask cleanup commands completions config deps desc doctor fetch formulae gist-logs homepage info leaves link log migrate missing options outdated pin pin readall reinstall shellenv tap-info tap unlink unpin untap update-reset update upgrade uses" -l "caskroom" -d "Display Homebrew\180s Caskroom path." -r
complete -c brew -n "not __fish_seen_subcommand_from install uninstall list search analytics autoremove cask cleanup commands completions config deps desc doctor fetch formulae gist-logs homepage info leaves link log migrate missing options outdated pin pin readall reinstall shellenv tap-info tap unlink unpin untap update-reset update upgrade uses" -l "cellar" -d "Display Homebrew\180s Cellar path." -r
complete -c brew -n "not __fish_seen_subcommand_from install uninstall list search analytics autoremove cask cleanup commands completions config deps desc doctor fetch formulae gist-logs homepage info leaves link log migrate missing options outdated pin pin readall reinstall shellenv tap-info tap unlink unpin untap update-reset update upgrade uses" -l "env" -d "Summarise Homebrew\180s build environment as a plain list." -x
...
complete -k -c brew -n __fish_use_subcommand -x -a unlink -d "Remove symlinks for formula from Homebrew\180s prefix. This can be useful for temporarily disabling a formula: brew unlink formula && commands && brew link formula"
...
complete -k -c brew -n __fish_use_subcommand -x -a link -d "Symlink all of formula\180s installed files into Homebrew\180s prefix. This is done automatically when you install formulae but can be useful for DIY installations."
...
complete -k -c brew -n __fish_use_subcommand -x -a homepage -d "Open a formula or cask\180s homepage in a browser, or open Homebrew\180s own homepage if no argument is provided."
...
complete -k -c brew -n __fish_use_subcommand -x -a doctor -d "Check your system for potential problems. Will exit with a non-zero status if any potential problems are found. Please note that these warnings are just used to help the Homebrew maintainers with debugging if you file an issue. If everything you use Homebrew for is working fine: please don\180t worry or file an issue; just ignore this."
complete -k -c brew -n __fish_use_subcommand -x -a desc -d "Display formula\180s name and one-line description. Formula descriptions are cached; the cache is created on the first search, making that search slower than subsequent ones."
...
complete -k -c brew -n __fish_use_subcommand -x -a analytics -d "Control Homebrew\180s anonymous aggregate user behaviour analytics. Read more at https://docs.brew.sh/Analytics."
...
complete -c brew -n "__fish_seen_subcommand_from install" -l "cc" -d "Attempt to compile using the specified compiler, which should be the name of the compiler\180s executable, e.g. gcc-7 for GCC 7."
...
complete -c brew -n "__fish_seen_subcommand_from uninstall" -l "ignore-dependencies" -d "Don\180t fail uninstall, even if formula is a dependency of any installed formulae."
...
complete -c brew -n "__fish_seen_subcommand_from search" -l "macports" -d "Search for text in the given package manager\180s list."
complete -c brew -n "__fish_seen_subcommand_from search" -l "fink" -d "Search for text in the given package manager\180s list."
complete -c brew -n "__fish_seen_subcommand_from search" -l "opensuse" -d "Search for text in the given package manager\180s list."
complete -c brew -n "__fish_seen_subcommand_from search" -l "fedora" -d "Search for text in the given package manager\180s list."
complete -c brew -n "__fish_seen_subcommand_from search" -l "debian" -d "Search for text in the given package manager\180s list."
complete -c brew -n "__fish_seen_subcommand_from search" -l "ubuntu" -d "Search for text in the given package manager\180s list."
...
complete -c brew -n "__fish_seen_subcommand_from readall" -l "syntax" -d "Syntax-check all of Homebrew\180s Ruby files (if no <tap> is passed)."
