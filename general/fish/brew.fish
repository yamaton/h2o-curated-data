...
complete -c brew -n "not __fish_seen_subcommand_from install uninstall list search analytics autoremove cask cleanup commands completions config deps desc doctor fetch formulae gist-logs homepage info leaves link log migrate missing options outdated pin pin readall reinstall shellenv tap-info tap unlink unpin untap update-reset update upgrade uses" -l caskroom -d 'Display Homebrew´s Caskroom path.' -x
complete -c brew -n "not __fish_seen_subcommand_from install uninstall list search analytics autoremove cask cleanup commands completions config deps desc doctor fetch formulae gist-logs homepage info leaves link log migrate missing options outdated pin pin readall reinstall shellenv tap-info tap unlink unpin untap update-reset update upgrade uses" -l cellar -d 'Display Homebrew´s Cellar path.' -x
...
complete -c brew -n __fish_use_subcommand -x -a install -d 'Install a formula or cask. Additional options specific to a formula may be appended to the command.'
complete -c brew -n __fish_use_subcommand -x -a uninstall -d 'Uninstall a formula or cask.'
complete -c brew -n __fish_use_subcommand -x -a list -d 'List all installed formulae and casks.'
complete -c brew -n __fish_use_subcommand -x -a search -d 'Perform a substring search of cask tokens and formula names for text. If text is flanked by slashes, it is interpreted as a regular expression. The search for text is extended online to homebrew/core and homebrew/cask.'
complete -c brew -n __fish_use_subcommand -x -a analytics -d 'Control Homebrew´s anonymous aggregate user behaviour analytics. Read more at https://docs.brew.sh/Analytics.'
complete -c brew -n __fish_use_subcommand -x -a autoremove -d 'Uninstall formulae that were only installed as a dependency of another formula and are now no longer needed.'
complete -c brew -n __fish_use_subcommand -x -a cask -d 'List only casks, or treat all named arguments as casks.'
complete -c brew -n __fish_use_subcommand -x -a cleanup -d 'Remove stale lock files and outdated downloads for all formulae and casks, and remove old versions of installed formulae. If arguments are specified, only do this for the given formulae and casks. Removes all downloads more than 120 days old. This can be adjusted with HOMEBREW_CLEANUP_MAX_AGE_DAYS.'
complete -c brew -n __fish_use_subcommand -x -a commands -d 'Show lists of built-in and external commands.'
complete -c brew -n __fish_use_subcommand -x -a completions -d 'Control whether Homebrew automatically links external tap shell completion files. Read more at https://docs.brew.sh/Shell-Completion .'
complete -c brew -n __fish_use_subcommand -x -a config -d 'Show Homebrew and system configuration info useful for debugging. If you file a bug report, you will be required to provide this information.'
complete -c brew -n __fish_use_subcommand -x -a deps -d 'Show dependencies for formula. Additional options specific to formula may be appended to the command. When given multiple formula arguments, show the intersection of dependencies for each formula.'
complete -c brew -n __fish_use_subcommand -x -a desc -d 'Display formula´s name and one-line description. Formula descriptions are cached; the cache is created on the first search, making that search slower than subsequent ones.'
complete -c brew -n __fish_use_subcommand -x -a doctor -d 'Check your system for potential problems. Will exit with a non-zero status if any potential problems are found. Please note that these warnings are just used to help the Homebrew maintainers with debugging if you file an issue. If everything you use Homebrew for is working fine: please don´t worry or file an issue; just ignore this.'
complete -c brew -n __fish_use_subcommand -x -a fetch -d 'Download a bottle (if available) or source packages for formulae and binaries for casks. For files, also print SHA-256 checksums.'
complete -c brew -n __fish_use_subcommand -x -a formulae -d 'List all locally installable formulae including short names.'
complete -c brew -n __fish_use_subcommand -x -a gist-logs -d 'Upload logs for a failed build of formula to a new Gist. Presents an error message if no logs are found.'
complete -c brew -n __fish_use_subcommand -x -a homepage -d 'Open a formula or cask´s homepage in a browser, or open Homebrew´s own homepage if no argument is provided.'
complete -c brew -n __fish_use_subcommand -x -a info -d 'Display brief statistics for your Homebrew installation.'
complete -c brew -n __fish_use_subcommand -x -a leaves -d 'List installed formulae that are not dependencies of another installed formula.'
complete -c brew -n __fish_use_subcommand -x -a link -d 'Symlink all of formula´s installed files into Homebrew´s prefix. This is done automatically when you install formulae but can be useful for DIY installations.'
complete -c brew -n __fish_use_subcommand -x -a log -d 'Show the git log for formula, or show the log for the Homebrew repository if no formula is provided.'
complete -c brew -n __fish_use_subcommand -x -a migrate -d 'Migrate renamed packages to new names, where formula are old names of packages.'
complete -c brew -n __fish_use_subcommand -x -a missing -d 'Check the given formula kegs for missing dependencies. If no formula are provided, check all kegs. Will exit with a non-zero status if any kegs are found to be missing dependencies.'
complete -c brew -n __fish_use_subcommand -x -a options -d 'Show install options specific to formula.'
complete -c brew -n __fish_use_subcommand -x -a outdated -d 'List installed casks and formulae that have an updated version available. By default, version information is displayed in interactive shells, and suppressed otherwise.'
complete -c brew -n __fish_use_subcommand -x -a pin -d 'Pin the specified formula, preventing them from being upgraded when issuing the brew upgrade formula command. See also unpin.'
complete -c brew -n __fish_use_subcommand -x -a pin -d 'Rerun the post-install steps for formula.'
complete -c brew -n __fish_use_subcommand -x -a readall -d 'Import all items from the specified tap, or from all installed taps if none is provided. This can be useful for debugging issues across all items when making significant changes to formula.rb, testing the performance of loading all items or checking if any current formulae/casks have Ruby issues.'
complete -c brew -n __fish_use_subcommand -x -a reinstall -d 'Uninstall and then reinstall a formula or cask using the same options it was originally installed with, plus any appended options specific to a formula.'
complete -c brew -n __fish_use_subcommand -x -a shellenv -d 'Print export statements. When run in a shell, this installation of Homebrew will be added to your PATH, MANPATH, and INFOPATH.'
complete -c brew -n __fish_use_subcommand -x -a tap-info -d 'Show detailed information about one or more taps.'
complete -c brew -n __fish_use_subcommand -x -a tap -d 'Tap a formula repository.'
complete -c brew -n __fish_use_subcommand -x -a unlink -d 'Remove symlinks for formula from Homebrew´s prefix. This can be useful for temporarily disabling a formula: brew unlink formula && commands && brew link formula'
complete -c brew -n __fish_use_subcommand -x -a unpin -d 'Unpin formula, allowing them to be upgraded by brew upgrade formula. See also pin.'
complete -c brew -n __fish_use_subcommand -x -a untap -d 'Remove a tapped formula repository.'
complete -c brew -n __fish_use_subcommand -x -a update-reset -d 'Fetch and reset Homebrew and all tap repositories (or any specified repository) using git(1) to their latest origin/HEAD.'
complete -c brew -n __fish_use_subcommand -x -a update -d 'Fetch the newest version of Homebrew and all formulae from GitHub using git(1) and perform any necessary migrations.'
complete -c brew -n __fish_use_subcommand -x -a upgrade -d 'Upgrade outdated casks and outdated, unpinned formulae using the same options they were originally installed with, plus any appended brew formula options. If cask or formula are specified, upgrade only the given cask or formula kegs (unless they are pinned; see pin, unpin)'
complete -c brew -n __fish_use_subcommand -x -a uses -d 'Show formulae and casks that specify formula as a dependency; that is, show dependents of formula. When given multiple formula arguments, show the intersection of formulae that use formula. By default, uses shows all formulae and casks that specify formula as a required or recommended dependency for their stable builds.'
