...
complete -c convert -o affine -d 'affine transform matrix' -x
complete -c convert -o alpha -d 'activate, deactivate, reset, or set the alpha channel' -x
complete -c convert -o antialias -d 'remove pixel-aliasing'
complete -c convert -o authenticate -d 'decipher image with this password' -x
complete -c convert -o attenuate -d 'lessen (or intensify) when adding noise to an image' -x
complete -c convert -o background -d 'background color' -x
complete -c convert -o bias -d 'add bias when convolving an image' -x
...
complete -c convert -o compress -d 'type of pixel compression when writing the image' -x
complete -c convert -o define -d 'define one or more image format options' -x
complete -c convert -o delay -d 'display the next image after pausing' -x
complete -c convert -o density -d 'horizontal and vertical density of the image' -x
complete -c convert -o depth -d 'image depth' -x
complete -c convert -o direction -d 'render text right-to-left or left-to-right' -x
complete -c convert -o display -d 'get image or font from this X server' -x
complete -c convert -o dispose -d 'layer disposal method' -x
complete -c convert -o dither -d 'apply error diffusion to image' -x
...
complete -c convert -o format -d 'output formatted image characteristics' -x
...
complete -c convert -o interpolate -d 'pixel color interpolation method' -x
complete -c convert -o interword-spacing -d 'set the space between two words' -x
complete -c convert -o kerning -d 'set the space between two letters' -x
...
complete -c convert -o orient -d 'image orientation' -x
complete -c convert -o page -d 'size and location of an image canvas (setting)' -x
complete -c convert -o ping -d 'efficiently determine image attributes'
complete -c convert -o pointsize -d 'font point size' -x
complete -c convert -o precision -d 'maximum number of significant digits to print' -x
...
complete -c convert -o size -d 'width and height of image' -x
complete -c convert -o stretch -d 'render text with this font stretch' -x
complete -c convert -o stroke -d 'graphic primitive stroke color' -x
complete -c convert -o strokewidth -d 'graphic primitive stroke width' -x
complete -c convert -o style -d 'render text with this font style' -x
complete -c convert -o support -d 'resize support: > 1.0 is blurry, < 1.0 is sharp' -x
...
complete -c convert -o texture -d 'name of texture to tile onto the image background' -r
complete -c convert -o tile-offset -d 'tile offset' -x
complete -c convert -o treedepth -d 'color tree depth' -x
complete -c convert -o transparent-color -d 'transparent color' -x
complete -c convert -o undercolor -d 'annotation bounding box color' -x
complete -c convert -o units -d 'the units of image resolution' -x
...
complete -c convert -o alpha -d 'on, activate, off, deactivate, set, opaque, copy transparent, extract, background, or shape' -x
complete -c convert -o annotate -d 'annotate the image with text' -x
...
complete -c convert -o border -d 'surround image with a border of color' -x
...
complete -c convert -o emboss -d 'emboss an image' -x
...
complete -c convert -o frame -d 'surround image with an ornamental border' -x
complete -c convert -o function -d 'apply function over image values' -x
complete -c convert -o gamma -d 'level of gamma correction' -x
complete -c convert -o gaussian-blur -d 'reduce image noise and reduce detail levels' -x
complete -c convert -o geometry -d 'preferred size or location of the image' -x
complete -c convert -o grayscale -d 'convert image to grayscale' -x
complete -c convert -o hough-lines -d 'identify lines in the image' -x
...
complete -c convert -o modulate -d 'vary the brightness, saturation, and hue' -x
complete -c convert -o monochrome -d 'transform image to black and white'
complete -c convert -o morphology -d 'apply a morphology method to the image' -x
complete -c convert -o motion-blur -d 'simulate motion blur' -x
complete -c convert -o negate -d 'replace every pixel with its complementary color'
complete -c convert -o noise -d 'add or reduce noise in an image' -x
complete -c convert -o normalize -d 'transform image to span the full range of colors'
complete -c convert -o opaque -d 'change this color to the fill color' -x
complete -c convert -o ordered-dither -d 'add a noise pattern to the image with specific amplitudes' -x
complete -c convert -o paint -d 'simulate an oil painting' -x
complete -c convert -o perceptible -d 'pixel value less than |epsilon| become epsilon or -epsilon' -x
complete -c convert -o polaroid -d 'simulate a Polaroid picture' -x
...
complete -c convert -o resize -d 'resize the image' -x
complete -c convert -o roll -d 'roll an image vertically or horizontally' -x
complete -c convert -o rotate -d 'apply Paeth rotation to the image' -x
complete -c convert -o rotational-blur -d 'rotational blur the image' -x
complete -c convert -o sample -d 'scale image with pixel sampling' -x
complete -c convert -o scale -d 'scale the image' -x
complete -c convert -o segment -d 'segment an image' -x
complete -c convert -o selective-blur -d 'selectively blur pixels within a contrast threshold' -x
complete -c convert -o sepia-tone -d 'simulate a sepia-toned photo' -x
complete -c convert -o set -d 'set an image property' -x
complete -c convert -o shade -d 'shade the image using a distant light source' -x
complete -c convert -o shadow -d 'simulate an image shadow' -x
complete -c convert -o sharpen -d 'sharpen the image' -x
complete -c convert -o shave -d 'shave pixels from the image edges' -x
complete -c convert -o shear -d 'slide one edge of the image along the X or Y axis' -x
...
complete -c convert -o statistic -d 'replace each pixel with corresponding statistic from the neighborhood' -x
complete -c convert -o strip -d 'strip image of all profiles and comments'
complete -c convert -o swirl -d 'swirl image pixels about the center' -x
complete -c convert -o threshold -d 'threshold the image' -x
complete -c convert -o thumbnail -d 'create a thumbnail of the image' -x
complete -c convert -o tile -d 'tile image when filling a graphic primitive' -r
complete -c convert -o tint -d 'tint the image with the fill color' -x
complete -c convert -o transform -d 'affine transform image'
complete -c convert -o transparent -d 'make this color transparent within the image' -x
...
complete -c convert -o type -d 'image type' -x
complete -c convert -o unique-colors -d 'discard all but one of any pixel color'
complete -c convert -o unsharp -d 'sharpen the image' -x
...
complete -c convert -o compare -d 'mathematically and visually annotate the difference between an image and its reconstruction'
complete -c convert -o complex -d 'perform complex mathematics on an image sequence' -x
complete -c convert -o composite -d 'composite image'
complete -c convert -o copy -d 'copy pixels from one area of an image to another' -x
complete -c convert -o crop -d 'cut out a rectangular region of the image' -x
complete -c convert -o deconstruct -d 'break down an image sequence into constituent parts'
complete -c convert -o evaluate-sequence -d 'evaluate an arithmetic, relational, or logical expression' -x
complete -c convert -o flatten -d 'flatten a sequence of images'
complete -c convert -o fx -d 'apply mathematical expression to an image channel(s)' -x
complete -c convert -o hald-clut -d 'apply a Hald color lookup table to the image'
complete -c convert -o layers -d 'optimize, merge, or compare image layers' -x
complete -c convert -o morph -d 'morph an image sequence' -x
...
complete -c convert -o delete -d 'delete the image from the image sequence' -x
complete -c convert -o duplicate -d 'duplicate an image one or more times' -x
complete -c convert -o insert -d 'insert last image into the image sequence' -x
complete -c convert -o reverse -d 'reverse image sequence'
complete -c convert -o swap -d 'swap two images in the image sequence' -x
complete -c convert -o debug -d 'display copious debugging information' -x
complete -c convert -o distribute-cache -d 'distributed pixel cache spanning one or more servers' -x
complete -c convert -o help -d 'print program options'
complete -c convert -o list -d 'print a list of supported option arguments' -x
complete -c convert -o log -d 'format of debugging information' -x
