...
complete -c curl -l "alt-svc" -d "(HTTPS) WARNING: this option is experimental." -r
...
complete -c curl -l "compressed" -d "(HTTP) Request a compressed response using one of the algorithms curl supports, and save the uncompressed document."
...
complete -c curl -l "connect-to" -d "For a request to the given HOST1:PORT1 pair, connect to HOST2:PORT2 instead." -r
...
complete -c curl -s "d" -l "data" -d "(HTTP) Sends the specified data in a POST request to the HTTP server, in the same way that a browser does when a user has filled in an HTML form and presses the submit button." -x
...
complete -c curl -l "disable-epsv" -d "(FTP) (FTP) Tell curl to disable the use of the EPSV command when doing passive FTP transfers."
...
complete -c curl -l "doh-url" -d "(all) Specifies which DNS-over-HTTPS (DOH) server to use to resolve hostnames, instead of using the default name resolver mechanism." -x
...
complete -c curl -l "etag-compare" -d "(HTTP) This option makes a conditional HTTP request for the specific ETag read from the given file by sending a custom If-NoneMatch header using the extracted ETag." -r
...
complete -c curl -l "ftp-create-dirs" -d "(FTP SFTP) When an FTP or SFTP URL/operation uses a path that doesn't currently exist on the server, the standard behavior of curl is to fail."
...
complete -c curl -l "happy-eyeballs-timeout-ms" -d "Happy eyeballs is an algorithm that attempts to connect to both IPv4 and IPv6 addresses for dual-stack hosts, preferring IPv6 first for the number of milliseconds." -x
...
complete -c curl -l "libcurl" -d "Append this option to any ordinary curl command line, and you will get a libcurl-using C source code written to the file that does the equivalent of what your command-line operation does!" -r
...
complete -c curl -l "mail-rcpt" -d "(SMTP) Specify a single address, user name or mailing list name." -x
complete -c curl -s "M" -l "manual" -d "Manual."
complete -c curl -l "max-filesize" -d "Specify the maximum size (in bytes) of a file to download." -r
complete -c curl -l "max-redirs" -d "(HTTP) Set maximum number of redirection-followings allowed." -r
complete -c curl -s "m" -l "max-time" -d "Maximum time in seconds that you allow the whole operation to take." -x
complete -c curl -l "metalink" -d "This option can tell curl to parse and process a given URI as Metalink file (both version 3 and 4 (RFC 5854) are supported) and make use of the mirrors listed within for failover if there are errors (such as the file or server not being available)."
...
complete -c curl -l "netrc-optional" -d "Very similar to -n, --netrc, but this option makes the .netrc usage optional and not mandatory as the -n, --netrc option does."
...
complete -c curl -l "noproxy" -d "Comma-separated list of hosts which do not use a proxy, if one is specified." -x
...
complete -c curl -l "oauth2-bearer" -d "(IMAP POP3 SMTP) Specify the Bearer Token for OAUTH 2.0 server authentication." -x
...
complete -c curl -l "parallel-max" -d "When asked to do parallel transfers, using -Z, --parallel, this option controls the maximum amount of transfers to do simultaneously."
...
complete -c curl -l "proto" -d "Tells curl to limit what protocols it may use in the transfer." -x
...
complete -c curl -l "request-target" -d "(HTTP) Tells curl to use an alternative \"target\" (path) instead of using the path as provided in the URL."
...
complete -c curl -l "sasl-authzid" -d "Use this authorisation identity (authzid), during SASL PLAIN authentication, in addition to the authentication identity (authcid) as specified by -u, --user."
...
complete -c curl -s "2" -l "sslv2" -d "(SSL) Forces curl to use SSL version 2 when negotiating with a remote SSL server."
complete -c curl -s "3" -l "sslv3" -d "(SSL) Forces curl to use SSL version 3 when negotiating with a remote SSL server."
...
complete -c curl -l "suppress-connect-headers" -d "When -p, --proxytunnel is used and a CONNECT request is made don't output proxy CONNECT response headers."
...
complete -c curl -l "tlspassword" -d "Set password for use with the TLS authentication method specified with --tlsauthtype."
