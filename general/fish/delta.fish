...
complete -c delta -l "blame-code-style" -d "Style string for the code section of a git blame line." -x
complete -c delta -l "blame-format" -d "Format string for git blame commit metadata." -x
complete -c delta -l "blame-palette" -d "Background colors used for git blame lines (space-separated string)." -x
complete -c delta -l "blame-separator-format" -d "Separator between the blame format and the code section of a git blame line." -x
complete -c delta -l "blame-separator-style" -d "Style string for the blame-separator-format" -x
complete -c delta -l "blame-timestamp-format" -d "Format of `git blame` timestamp in raw git output received by delta" -x
complete -c delta -l "color-only" -d "Do not alter the input structurally in any way."
complete -c delta -l "commit-decoration-style" -d "Style string for the commit hash decoration." -x
complete -c delta -l "commit-regex" -d "Regular expression used to identify the commit line when parsing git output" -x
complete -c delta -l "commit-style" -d "Style string for the commit hash line." -x
...
complete -c delta -l "line-buffer-size" -d "Size of internal line buffer." -x
complete -c delta -l "line-fill-method" -d "Line-fill method in side-by-side mode." -x
...
complete -c delta -l "map-styles" -d "Map styles encountered in raw input to desired output styles." -x
complete -c delta -l "max-line-distance" -d "Maximum line pair distance parameter in within-line diff algorithm." -x
complete -c delta -l "max-line-length" -d "Truncate lines longer than this." -x
complete -c delta -l "merge-conflict-begin-symbol" -d "String marking the beginning of a merge conflict region." -x
complete -c delta -l "merge-conflict-end-symbol" -d "String marking the end of a merge conflict region." -x
complete -c delta -l "merge-conflict-ours-diff-header-decoration-style" -d "Style string for the decoration of the header above the 'ours' merge conflict diff." -x
complete -c delta -l "merge-conflict-ours-diff-header-style" -d "Style string for the header above the 'ours' branch merge conflict diff." -x
complete -c delta -l "merge-conflict-theirs-diff-header-decoration-style" -d "Style string for the decoration of the header above the 'theirs' merge conflict diff." -x
complete -c delta -l "merge-conflict-theirs-diff-header-style" -d "Style string for the header above the 'theirs' branch merge conflict diff." -x
complete -c delta -l "minus-empty-line-marker-style" -d "Style string for removed empty line marker." -x
complete -c delta -l "minus-emph-style" -d "Style string for emphasized sections of removed lines." -x
complete -c delta -l "minus-non-emph-style" -d "Style string for non-emphasized sections of removed lines that have an emphasized section." -x
complete -c delta -l "minus-style" -d "Style string for removed lines." -x
complete -c delta -l "navigate" -d "Activate diff navigation."
complete -c delta -l "navigate-regex" -d "Regular expression defining navigation stop points" -x
complete -c delta -l "no-gitconfig" -d "Do not read any settings from git config."
complete -c delta -l "pager" -d "Which pager to use." -x
complete -c delta -l "paging" -d "Whether to use a pager when displaying output." -x
complete -c delta -l "parse-ansi" -d "Display ANSI color escape sequences in human-readable form."
complete -c delta -l "plus-emph-style" -d "Style string for emphasized sections of added lines." -x
complete -c delta -l "plus-empty-line-marker-style" -d "Style string for added empty line marker." -x
complete -c delta -l "plus-non-emph-style" -d "Style string for non-emphasized sections of added lines that have an emphasized section." -x
...
complete -c delta -l "syntax-theme" -d "The syntax-highlighting theme to use." -x
complete -c delta -l "tabs" -d "The number of spaces to replace tab characters with." -x
complete -c delta -l "true-color" -d "Whether to emit 24-bit (\"true color\") RGB color codes." -x
complete -c delta -l "whitespace-error-style" -d "Style string for whitespace errors." -x
complete -c delta -s "w" -l "width" -d "The width of underline/overline decorations." -x
complete -c delta -l "word-diff-regex" -d "Regular expression defining a 'word' in within-line diff algorithm." -x
complete -c delta -l "wrap-left-symbol" -d "End-of-line wrapped content symbol (left-aligned)." -x
complete -c delta -l "wrap-max-lines" -d "How often a line should be wrapped if it does not fit." -x
complete -c delta -l "wrap-right-percent" -d "Threshold for right-aligning wrapped content." -x
complete -c delta -l "wrap-right-prefix-symbol" -d "Pre-wrapped content symbol (right-aligned)." -x
complete -c delta -l "wrap-right-symbol" -d "End-of-line wrapped content symbol (right-aligned)." -x
complete -c delta -l "zero-style" -d "Style string for unchanged lines." -x
complete -c delta -l "24-bit-color" -d "Deprecated: use --true-color" -x
complete -c delta -s "h" -l "help" -d "Print help information"
complete -c delta -s "V" -l "version" -d "Print version information"
