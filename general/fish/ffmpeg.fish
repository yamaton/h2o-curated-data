...
complete -c ffmpeg -o report -d 'Dump full command line and log output to a file named "program-YYYYMMDD-HHMMSS.log" in the current directory.'
...
complete -c ffmpeg -s f -d 'Force input or output file format.' -x
complete -c ffmpeg -s i -d 'input file url' -x
complete -c ffmpeg -s y -d 'Overwrite output files without asking.'
complete -c ffmpeg -s y -d 'Overwrite output files without asking.' -x
complete -c ffmpeg -s n -d 'Do not overwrite output files, and exit immediately if a specified output file already exists.'
complete -c ffmpeg -s n -d 'Do not overwrite output files, and exit immediately if a specified output file already exists.' -x
complete -c ffmpeg -o stream_loop -d 'Set number of times input stream shall be looped.' -x
complete -c ffmpeg -o codec -d 'Select an encoder (when used before an output file) or a decoder (when used before an input file) for one or more streams.' -x
complete -c ffmpeg -s t -d 'When used as an input option (before "-i"), limit the duration of data read from the input file.' -x
complete -c ffmpeg -o to -d 'Stop writing the output or reading the input at position.' -x
complete -c ffmpeg -o fs -d 'Set the file size limit, expressed in bytes.' -x
complete -c ffmpeg -o ss -d 'When used as an input option (before "-i"), seeks in this input file to position.' -x
complete -c ffmpeg -o sseof -d 'Like the "-ss" option but relative to the "end of file".' -x
...
complete -c ffmpeg -o disposition:stream_specifier -d 'Sets the disposition for a stream.' -x
...
complete -c ffmpeg -o target -d 'Specify target file type ("vcd", "svcd", "dvd", "dv", "dv50").' -x
complete -c ffmpeg -o dn -d 'As an input option, blocks all data streams of a file from being filtered or being automatically selected or mapped for any output.'
complete -c ffmpeg -o dn -d 'As an input option, blocks all data streams of a file from being filtered or being automatically selected or mapped for any output.' -x
...
complete -c ffmpeg -o stats -d 'Print encoding progress/statistics.'
...
complete -c ffmpeg -o debug_ts -d 'Print timestamp information.'
...
complete -c ffmpeg -o vn -d 'As an input option, blocks all video streams of a file from being filtered or being automatically selected or mapped for any output.'
complete -c ffmpeg -o vn -d 'As an input option, blocks all video streams of a file from being filtered or being automatically selected or mapped for any output.' -x
complete -c ffmpeg -o vcodec -d 'Set the video codec.' -x
complete -c ffmpeg -o pass -d 'Select the pass number (1 or 2).' -x
complete -c ffmpeg -o passlogfile -d 'Set two-pass log file name prefix to prefix, the default file name prefix is ``ffmpeg2pass\'\'.' -x
...
complete -c ffmpeg -o force_key_frames -d 'force_key_frames can take arguments of the following form:' -x
...
complete -c ffmpeg -o init_hw_device -d 'Initialise a new hardware device of type type called name, deriving it from the existing device with the name source.' -x
...
complete -c ffmpeg -o an -d 'As an input option, blocks all audio streams of a file from being filtered or being automatically selected or mapped for any output.'
complete -c ffmpeg -o an -d 'As an input option, blocks all audio streams of a file from being filtered or being automatically selected or mapped for any output.' -x
...
complete -c ffmpeg -o sn -d 'As an input option, blocks all subtitle streams of a file from being filtered or being automatically selected or mapped for any output.'
complete -c ffmpeg -o sn -d 'As an input option, blocks all subtitle streams of a file from being filtered or being automatically selected or mapped for any output.' -x
...
complete -c ffmpeg -o map -d 'Designate one or more input streams as a source for the output file.' -r
...
complete -c ffmpeg -o benchmark -d 'Show benchmarking information at the end of an encode.'
complete -c ffmpeg -o benchmark -d 'Show benchmarking information at the end of an encode.' -x
complete -c ffmpeg -o benchmark_all -d 'Show benchmarking information during the encode.'
complete -c ffmpeg -o benchmark_all -d 'Show benchmarking information during the encode.' -x
complete -c ffmpeg -o timelimit -d 'Exit after ffmpeg has been running for duration seconds in CPU user time.' -x
complete -c ffmpeg -o dump -d 'Dump each input packet to stderr.'
...
complete -c ffmpeg -o re -d 'Read input at native frame rate.'
complete -c ffmpeg -o re -d 'Read input at native frame rate.' -x
...
complete -c ffmpeg -o bitexact -d 'Enable bitexact mode for (de)muxer and (de/en)coder'
complete -c ffmpeg -o bitexact -d 'Enable bitexact mode for (de)muxer and (de/en)coder' -x
complete -c ffmpeg -o shortest -d 'Finish encoding when the shortest input stream ends.'
...
complete -c ffmpeg -o streamid -d 'Assign a new stream-id value to an output stream.' -x
...
complete -c ffmpeg -o accurate_seek -d 'This option enables or disables accurate seeking in input files with the -ss option.'
complete -c ffmpeg -o accurate_seek -d 'This option enables or disables accurate seeking in input files with the -ss option.' -x
complete -c ffmpeg -o seek_timestamp -d 'This option enables or disables seeking by timestamp in input files with the -ss option.'
complete -c ffmpeg -o seek_timestamp -d 'This option enables or disables seeking by timestamp in input files with the -ss option.' -x
complete -c ffmpeg -o thread_queue_size -d 'This option sets the maximum number of queued packets when reading from the file or device.' -x
complete -c ffmpeg -o sdp_file -d 'Print sdp information for an output stream to file.' -r
complete -c ffmpeg -o discard -d 'Allows discarding specific streams or frames from streams.'
...
complete -c ffmpeg -o xerror -d 'Stop and exit on error'
