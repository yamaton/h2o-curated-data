...
complete -c find -o maxdepth -d 'Descend at most levels (a non-negative integer) levels of directories below the starting-points.' -x
complete -c find -o mindepth -d 'Do not apply any tests or actions at levels less than levels (a non-negative integer).' -x
...
complete -c find -o amin -d 'File was last accessed n minutes ago.' -x
complete -c find -o anewer -d 'Time of the last access of the current file is more recent than that of the last data modification of the reference file.' -x
complete -c find -o atime -d 'File was last accessed n*24 hours ago.' -x
complete -c find -o cmin -d 'File\'s status was last changed n minutes ago.' -x
complete -c find -o cnewer -d 'Time of the last status change of the current file is more recent than that of the last data modification of the reference file.' -x
complete -c find -o ctime -d 'File\'s status was last changed n*24 hours ago.' -x
...
complete -c find -o fstype -d 'File is on a filesystem of type type.' -x
complete -c find -o gid -d 'File\'s numeric group ID is n.' -x
complete -c find -o group -d 'File belongs to group gname (numeric group ID allowed).' -x
complete -c find -o ilname -d 'Like -lname, but the match is case insensitive.' -x
complete -c find -o iname -d 'Like -name, but the match is case insensitive.' -x
complete -c find -o inum -d 'File has inode number n.' -x
complete -c find -o ipath -d 'Like -path.' -x
complete -c find -o iregex -d 'Like -regex, but the match is case insensitive.' -x
complete -c find -o iwholename -d 'See -ipath.' -x
complete -c find -o links -d 'File has n hard links.' -x
complete -c find -o lname -d 'File is a symbolic link whose contents match shell pattern pattern.' -x
complete -c find -o mmin -d 'File\'s data was last modified n minutes ago.' -x
complete -c find -o mtime -d 'File\'s data was last modified n*24 hours ago.' -x
complete -c find -o name -d 'Base of file name (the path with the leading directories removed) matches shell pattern pattern.' -x
complete -c find -o newer -d 'Time of the last data modification of the current file is more recent than that of the last data modification of the reference file.' -x
complete -c find -o newerXY -d 'Succeeds if timestamp X of the file being considered is newer than timestamp Y of the file reference.' -x
...
complete -c find -o path -d 'File name matches shell pattern pattern.' -x
complete -c find -o perm -d 'File\'s permission bits are exactly mode (octal or symbolic).' -x
complete -c find -o perm -d 'All of the permission bits mode are set for the file.' -x
complete -c find -o perm -d 'Any of the permission bits mode are set for the file.' -x
...
complete -c find -o regex -d 'File name matches regular expression pattern.' -x
complete -c find -o samefile -d 'File refers to the same inode as name.' -x
complete -c find -o size -d 'File uses n units of space, rounding up.' -x
complete -c find -o true -d 'Always true.'
complete -c find -o type -d 'File is of type c:' -x
complete -c find -o uid -d 'File\'s numeric user ID is n.' -x
complete -c find -o used -d 'File was last accessed n days after its status was last changed.' -x
complete -c find -o user -d 'File is owned by user uname (numeric user ID allowed).' -x
complete -c find -o wholename -d 'See -path.' -x
complete -c find -o writable -d 'Matches files which are writable by the current user.'
complete -c find -o xtype -d 'The same as -type unless the file is a symbolic link.' -x
complete -c find -o context -d '(SELinux only) Security context of the file matches glob pattern.' -x
complete -c find -o delete -d 'Delete files; true if removal succeeded.'
complete -c find -o exec -d 'Execute command; true if 0 status is returned.' -x
complete -c find -o exec -d 'This variant of the -exec action runs the specified command on the selected files, but the command line is built by appending each selected file name at the end; the total number of invocations of the command will be much less than the number of matched files.' -x
complete -c find -o execdir -d 'Like -exec, but the specified command is run from the subdirectory containing the matched file, which is not normally the directory in which you started find.' -x
...
complete -c find -o ok -d 'Like -exec but ask the user first.' -x
complete -c find -o okdir -d 'Like -execdir but ask the user first in the same way as for -ok.' -x
...
complete -c find -o printf -d 'True; print format on the standard output, interpreting `\\' escapes and `%\' directives.' -x
