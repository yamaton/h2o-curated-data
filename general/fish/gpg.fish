...
complete -c gpg -l "help" -s "h" -d "Print a usage message summarizing the most useful command-line options."
...
complete -c gpg -l "clear-sign" -l "clearsign" -d "Make a cleartext signature."
complete -c gpg -l "detach-sign" -s "b" -d "Make a detached signature."
complete -c gpg -l "encrypt" -s "e" -d "Encrypt data to one or more public keys."
complete -c gpg -l "symmetric" -s "c" -d "Encrypt with a symmetric cipher using a passphrase."
complete -c gpg -l "store" -d "Store only (make a simple literal data packet)."
complete -c gpg -l "decrypt" -s "d" -d "Decrypt the file given on the command line (or STDIN if no file is specified) and write it to STDOUT (or the file specified with --output)."
...
complete -c gpg -l "list-keys" -s "k" -l "list-public-keys" -d "List the specified keys."
complete -c gpg -l "list-secret-keys" -s "K" -d "List the specified secret keys."
complete -c gpg -l "check-signatures" -l "check-sigs" -d "Same as --list-keys, but the key signatures are verified and listed too."
complete -c gpg -l "locate-keys" -l "locate-external-keys" -d "Locate the keys given as arguments."
...
complete -c gpg -l "edit-card" -l "card-edit" -d "Present a menu to work with a smartcard."
...
complete -c gpg -l "export-secret-keys" -l "export-secret-subkeys" -d "Same as --export, but exports the secret keys instead."
complete -c gpg -l "export-ssh-key" -d "This command is used to export a key in the OpenSSH public key format."
complete -c gpg -l "import" -l "fast-import" -d "Import/merge keys."
complete -c gpg -l "receive-keys" -l "recv-keys" -d "Import the keys with the given keyIDs from a keyserver." -x
...
complete -c gpg -l "print-md" -l "print-mds" -d "Print message digest of algorithm algo for all given files or STDIN." -r
...
complete -c gpg -l "enarmor" -l "dearmor" -d "Pack or unpack an arbitrary input into/from an OpenPGP ASCII armor."
complete -c gpg -l "tofu-policy" -d "Set the TOFU policy for all the bindings associated with the specified keys." -x
complete -c gpg -l "quick-generate-key" -l "quick-gen-key" -d "This is a simple command to generate a standard key with one user id." -r
complete -c gpg -l "quick-set-expire" -d "With two arguments given, directly set the expiration time of the primary key identified by fpr to expire." -r
complete -c gpg -l "quick-add-key" -d "Directly add a subkey to the key identified by the fingerprint fpr." -r
complete -c gpg -l "generate-key" -l "gen-key" -d "Generate a new key pair using the current default parameters."
complete -c gpg -l "full-generate-key" -l "full-gen-key" -d "Generate a new key pair with dialogs for all options."
complete -c gpg -l "generate-revocation" -l "gen-revoke" -d "Generate a revocation certificate for the complete key." -x
complete -c gpg -l "generate-designated-revocation" -l "desig-revoke" -d "Generate a designated revocation certificate for a key." -x
...
complete -c gpg -l "batch" -l "no-batch" -d "Use batch mode."
...
complete -c gpg -l "enable-large-rsa" -l "disable-large-rsa" -d "With --generate-key and --batch, enable the creation of RSA secret keys as large as 8192 bit."
complete -c gpg -l "enable-dsa2" -l "disable-dsa2" -d "Enable hash truncation for all DSA keys even for old DSA Keys up to 1024 bit."
complete -c gpg -l "photo-viewer" -d "This is the command line that should be run to view a photo ID." -r
complete -c gpg -l "exec-path" -d "Sets a list of directories to search for photo viewers If not provided photo viewers use the PATH environment variable." -r
...
complete -c gpg -l "utf8-strings" -l "no-utf8-strings" -d "Assume that command line arguments are given as UTF-8 strings."
...
complete -c gpg -s "z" -l "compress-level" -l "bzip2-compress-level" -d "Set compression level to n for the ZIP and ZLIB compression algorithms." -x
complete -c gpg -l "bzip2-decompress-lowmem" -d "Use a different decompression method for BZIP2 compressed files."
complete -c gpg -l "mangle-dos-filenames" -l "no-mangle-dos-filenames" -d "Older version of Windows cannot handle filenames with more than one dot."
...
complete -c gpg -l "auto-check-trustdb" -l "no-auto-check-trustdb" -d "If GnuPG feels that its information about the Web of Trust has to be updated, it automatically runs the --check-trustdb command internally."
complete -c gpg -l "use-agent" -l "no-use-agent" -d "This is dummy option."
...
complete -c gpg -l "limit-card-insert-tries" -d "With n greater than 0 the number of prompts asking to insert a smartcard gets limited to N-1." -r
...
complete -c gpg -l "require-secmem" -l "no-require-secmem" -d "Refuse to run if GnuPG cannot get secure memory."
complete -c gpg -l "require-cross-certification" -l "no-require-cross-certification" -d "When verifying a signature made from a subkey, ensure that the cross certification \"back signature\" on the subkey is present and valid."
complete -c gpg -l "expert" -l "no-expert" -d "Allow the user to do certain nonsensical or \"silly\" things like signing an expired or revoked key, or certain potentially incompatible things like generating unusual key types."
complete -c gpg -l "recipient" -s "r" -d "Encrypt for user id name." -x
complete -c gpg -l "hidden-recipient" -s "R" -d "Encrypt for user ID name, but hide the key ID of this user's key." -x
complete -c gpg -l "recipient-file" -s "f" -d "This option is similar to --recipient except that it encrypts to a key stored in the given file." -r
complete -c gpg -l "hidden-recipient-file" -s "F" -d "This option is similar to --hidden-recipient except that it encrypts to a key stored in the given file." -r
complete -c gpg -l "encrypt-to" -d "Same as --recipient but this one is intended for use in the options file and may be used with your own user-id as an \"encrypt-to-self\"." -r
complete -c gpg -l "hidden-encrypt-to" -d "Same as --hidden-recipient but this one is intended for use in the options file and may be used with your own user-id as a hidden \"encrypt-to-self\"." -r
...
complete -c gpg -l "local-user" -s "u" -d "Use name as the key to sign with." -x
complete -c gpg -l "sender" -d "This option has two purposes." -r
...
complete -c gpg -l "skip-hidden-recipients" -l "no-skip-hidden-recipients" -d "During decryption skip all anonymous recipients."
complete -c gpg -l "armor" -s "a" -d "Create ASCII armored output."
complete -c gpg -l "no-armor" -d "Assume the input data is not in ASCII armored format."
complete -c gpg -l "output" -s "o" -d "Write output to file." -r
complete -c gpg -l "max-output" -d "This option sets a limit on the number of bytes that will be generated when processing a file." -r
complete -c gpg -l "input-size-hint" -d "This option can be used to tell GPG the size of the input data in bytes." -r
complete -c gpg -l "key-origin" -d "gpg can track the origin of a key." -r
complete -c gpg -l "import-options" -d "This is a space or comma delimited string that gives options for importing keys." -x
complete -c gpg -l "import-filter" -l "export-filter" -d "These options define an import/export filter which are applied to the imported/exported keyblock right before it will be stored/written." -x
...
complete -c gpg -s "t" -l "textmode" -l "no-textmode" -d "Treat input files as text and store them in the OpenPGP canonical text form with standard \"CRLF\" line endings."
...
complete -c gpg -s "i" -l "interactive" -d "Prompt before overwriting any files."
...
complete -c gpg -l "status-fd" -d "Write special status strings to the file descriptor n." -r
complete -c gpg -l "status-file" -d "Same as --status-fd, except the status data is written to file file." -r
complete -c gpg -l "logger-fd" -d "Write log output to file descriptor n and not to STDERR." -r
complete -c gpg -l "log-file" -l "logger-file" -d "Same as --logger-fd, except the logger data is written to file file." -r
complete -c gpg -l "attribute-fd" -d "Write attribute subpackets to the file descriptor n." -r
complete -c gpg -l "attribute-file" -d "Same as --attribute-fd, except the attribute data is written to file file." -r
complete -c gpg -l "comment" -l "no-comments" -d "Use string as a comment string in cleartext signatures and ASCII armored messages or keys (see --armor)." -x
complete -c gpg -l "emit-version" -l "no-emit-version" -d "Force inclusion of the version string in ASCII armored output."
complete -c gpg -l "sig-notation" -l "cert-notation" -s "N" -l "set-notation" -d "Put the name value pair into the signature as notation data." -x
complete -c gpg -l "known-notation" -d "Adds name to a list of known critical signature notations." -x
complete -c gpg -l "sig-policy-url" -l "cert-policy-url" -l "set-policy-url" -d "Use string as a Policy URL for signatures (rfc4880:5.2.3.20)." -x
complete -c gpg -l "sig-keyserver-url" -d "Use string as a preferred keyserver URL for data signatures." -x
complete -c gpg -l "set-filename" -d "Use string as the filename which is stored inside messages." -r
complete -c gpg -l "for-your-eyes-only" -l "no-for-your-eyes-only" -d "Set the `for your eyes only' flag in the message."
complete -c gpg -l "use-embedded-filename" -l "no-use-embedded-filename" -d "Try to create a file with a name as embedded in the data."
...
complete -c gpg -l "throw-keyids" -l "no-throw-keyids" -d "Do not put the recipient key IDs into encrypted messages."
...
complete -c gpg -l "escape-from-lines" -l "no-escape-from-lines" -d "Because some mailers change lines starting with \"From \" to \">From \" it is good to handle such lines in a special way when creating cleartext signatures to prevent the mail system from breaking the signature."
complete -c gpg -l "passphrase-repeat" -d "Specify how many times gpg will request a new passphrase be repeated." -x
complete -c gpg -l "passphrase-fd" -d "Read the passphrase from file descriptor n." -r
...
complete -c gpg -l "command-fd" -d "This is a replacement for the deprecated shared-memory IPC mode." -r
complete -c gpg -l "command-file" -d "Same as --command-fd, except the commands are read out of file file" -r
complete -c gpg -l "allow-non-selfsigned-uid" -l "no-allow-non-selfsigned-uid" -d "Allow the import and use of keys with user IDs which are not self-signed."
...
complete -c gpg -l "list-signatures" -l "list-sigs" -d "Same as --list-keys, but the signatures are listed too."
...
complete -c gpg -l "override-session-key" -l "override-session-key-fd" -d "Don't use the public key but the session key string respective the session key taken from the first line read from file descriptor fd." -r
complete -c gpg -l "ask-sig-expire" -l "no-ask-sig-expire" -d "When making a data signature, prompt for an expiration time."
complete -c gpg -l "default-sig-expire" -d "The default expiration time to use for signature expiration."
complete -c gpg -l "ask-cert-expire" -l "no-ask-cert-expire" -d "When making a key signature, prompt for an expiration time."
complete -c gpg -l "default-cert-expire" -d "The default expiration time to use for key signature expiration."
complete -c gpg -l "default-new-key-algo" -d "This option can be used to change the default algorithms for key generation." -r
...
complete -c gpg -l "enable-special-filenames" -d "This option enables a mode in which filenames of the form \8216-&n\8217, where n is a non-negative decimal number, refer to the file descriptor n and not to a file with that name."
...
complete -c gpg -l "show-photos" -l "no-show-photos" -d "Causes --list-keys, --list-signatures, --list-public-keys, --list-secret-keys, and verifying a signature to also display the photo ID attached to the key, if any."
...
complete -c gpg -l "show-notation" -l "no-show-notation" -d "Show signature notations in the --list-signatures or --check-signatures listings as well as when verifying a signature with a notation in it."
complete -c gpg -l "show-policy-url" -l "no-show-policy-url" -d "Show policy URLs in the --list-signatures or --check-signatures listings as well as when verifying a signature with a policy URL in it."
complete -c gpg -l "force-v3-sigs" -d "--no-force-v3-sigs"
...
complete -c gpg -s "n" -d "True if value is not empty (no value allowed)."
complete -c gpg -s "z" -d "True if value is empty (no value allowed)."
complete -c gpg -s "t" -d "Alias for \"PROPNAME != 0\" (no value allowed)."
complete -c gpg -s "f" -d "Alias for \"PROPNAME == 0\" (no value allowed)."
complete -c gpg  -d "VALUE spans to the end of the expression."
complete -c gpg -s "c" -d "The string match in this part is done case-sensitive."
