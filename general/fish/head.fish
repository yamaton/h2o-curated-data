...
complete -c head -s "c" -l "bytes" -d "print the first NUM bytes of each file; with the leading '-', print all but the last NUM bytes of each file" -r
complete -c head -s "n" -l "lines" -d "print the first NUM lines instead of the first 10; with the leading '-', print all but the last NUM lines of each file" -r
