...
complete -c less -s b -l buffers -d 'Specifies the amount of buffer space less will use for each file, in units of kilobytes (1024 bytes).' -x
...
complete -c less -s h -l max-back-scroll -d 'Specifies a maximum number of lines to scroll backward.' -x
...
complete -c less -s j -l jump-target -d 'Specifies a line on the screen where the "target" line is to be positioned.' -x
...
complete -c less -s p -l pattern -d 'The -p option on the command line is equivalent to specifying +/pattern; that is, it tells less to start at the  first occurrence of pattern in the file.' -x
...
complete -c less -s t -l tag -d 'The -t option, followed immediately by a TAG, will edit the file containing that tag.' -x
...
complete -c less -s " -l quotes -d 'Changes the filename quoting character.' -x
