...
complete -c less -s "D" -l "color" -d "Changes the color of different parts of the displayed text." -x
...
complete -c less -s "k" -l "lesskey-file" -d "Causes less to open and interpret the named file as a lesskey(1) binary file." -r
complete -c less -l "lesskey-src" -d "Causes less to open and interpret the named file as a lesskey(1) source file." -r
...
complete -c less -s "R" -l "RAW-CONTROL-CHARS" -d "Like -r, but only ANSI \"color\" escape sequences and OSC 8 hyperlink sequences are output in \"raw\" form."
...
complete -c less -l "file-size" -d "If --file-size is specified, less will determine the size of the file immediately after opening the file."
...
complete -c less -l "incsearch" -d "Subsequent search commands will be \"incremental\"; that is, less will advance to the next line containing the search pattern as each character of the pattern is typed in."
complete -c less -l "line-num-width" -d "Sets the minimum width of the line number field when the -N option is in effect."
...
complete -c less -l "wheel-lines" -d "Set the number of lines to scroll when the mouse wheel is scrolled and the --mouse or --MOUSE option is in effect." -x
