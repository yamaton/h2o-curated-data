...
complete -c mv -s "S" -l "suffix" -d "override the usual backup suffix" -x
complete -c mv -s "t" -l "target-directory" -d "move all SOURCE arguments into DIRECTORY" -r
complete -c mv -s "T" -l "no-target-directory" -d "treat DEST as a normal file"
complete -c mv -s "u" -l "update" -d "move only when the SOURCE file is newer than the destination file or when the destination file is missing"
