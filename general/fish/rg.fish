...
complete -c rg -l color -d 'This flag controls when to use colors.' -x
...
complete -c rg -s E -l encoding -d 'Specify the text encoding that ripgrep will use on all files searched.' -x
complete -c rg -l engine -d 'Specify which regular expression engine to use.' -x
complete -c rg -l field-context-separator -d 'Set the field context separator, which is used to delimit file paths, line numbers, columns and the context itself, when printing contextual lines.' -x
complete -c rg -l field-match-separator -d 'Set the field match separator, which is used to delimit file paths, line numbers, columns and the match itself.' -x
...
complete -c rg -s g -l glob -d 'Include or exclude files and directories for searching that match the given glob.' -x
...
complete -c rg -s m -l max-count -d 'Limit the number of matching lines per file searched to NUM.' -x
complete -c rg -l max-depth -d 'Limit the depth of directory traversal to NUM levels beyond the paths given.' -x
complete -c rg -l max-filesize -d 'Ignore files larger than NUM in size.' -x
...
complete -c rg -l path-separator -d 'Set the path separator to use when printing file paths.' -x
...
complete -c rg -l pre -d 'For each input FILE, search the standard output of COMMAND FILE rather than the contents of FILE.' -x
complete -c rg -l pre-glob -d 'This flag works in conjunction with the --pre flag.' -x
...
complete -c rg -s r -l replace -d 'Replace every match with the text given when printing results.' -x
...
complete -c rg -s t -l type -d 'Only search files matching TYPE.' -x
complete -c rg -l type-add -d 'Add a new glob for a particular file type.' -x
complete -c rg -l type-clear -d 'Clear the file type globs previously defined for TYPE.' -x
complete -c rg -l type-list -d 'Show all supported file types and their corresponding globs.'
complete -c rg -s T -l type-not -d 'Do not search files matching TYPE.' -x
