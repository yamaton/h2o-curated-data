...
complete -c sort -l batch-size -d 'merge at most NMERGE inputs at once; for more use temp files' -x
...
complete -c sort -l files0-from -d 'read input from the files specified by NUL-terminated names in file F; If F is - then read names from standard input' -x
