...
complete -c ssh -s D -d 'Specifies a local “dynamic” application-level port forwarding.' -x
complete -c ssh -s E -d 'Append debug logs to log_file instead of standard error.' -r
complete -c ssh -s e -d 'Sets the escape character for sessions with a pty (default: ‘~’).' -x
...
complete -c ssh -s J -d 'Connect to the target host by first making a ssh connection to the jump host described by destination and then establishing a TCP forwarding to the ultimate destination from there.' -x
...
complete -c ssh -s l -d 'Specifies the user to log in as on the remote machine.' -x
...
complete -c ssh -s o -d 'Can be used to give options in the format used in the configuration file.' -x
complete -c ssh -s p -d 'Port to connect to on the remote host.' -x
...
complete -c ssh -s W -d 'Requests that standard input and output on the client be forwarded to host on port over the secure channel.' -x
