...
complete -c tcpdump -s i -l interface -d 'Listen on interface.' -x
...
complete -c tcpdump -s W -d 'Used in conjunction with the -C option, this will limit the number of files created to the specified number, and begin overwriting files from the beginning, thus creating a \'rotating\' buffer.'
