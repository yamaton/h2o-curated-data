...
complete -c unzip -s t -d 'test compressed archive data'
...
complete -c unzip -s q -d 'quiet mode (-qq => quieter)'
...
complete -c unzip -o aa -d 'treat ALL files as text'
...
complete -c unzip -s O -d 'specify a character encoding for DOS, Windows and OS/2 archives' -x
complete -c unzip -s I -d 'specify a character encoding for UNIX and other archives' -x
