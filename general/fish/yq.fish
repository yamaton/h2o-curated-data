...
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help" -l "header-preprocess" -d "Slurp any header comments and separators before processing expression."
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help" -s "h" -l "help" -d "help for yq"
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help" -s "I" -l "indent" -d "sets indent level for output (default 2)" -x
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help" -s "i" -l "inplace" -d "update the yaml file inplace of first yaml file given."
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help" -s "M" -l "no-colors" -d "force print with no colors"
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help" -s "N" -l "no-doc" -d "Don't print document separators (---)"
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help" -s "n" -l "null-input" -d "Don't read input, simply evaluate the expression given."
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help" -s "o" -l "output-format" -d "[yaml|y|json|j|props|p] output format type." -x
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help" -s "P" -l "prettyPrint" -d "pretty print, shorthand for '..."
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help" -l "unwrapScalar" -d "unwrap scalar, print the value with no quotes, colors or comments (default true)"
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help" -s "v" -l "verbose" -d "verbose mode"
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help" -s "V" -l "version" -d "Print version information and quit"
...
complete -c yq -n "__fish_seen_subcommand_from help" -l "header-preprocess" -d "Slurp any header comments and separators before processing expression."
