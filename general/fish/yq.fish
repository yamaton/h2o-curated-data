...
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -l "expression" -d "forcibly set the expression argument." -r
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -l "from-file" -d "Load expression from specified file." -r
...
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -l "header-preprocess" -d "Slurp any header comments and separators before processing expression."
...
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -s "i" -l "inplace" -d "update the file inplace of first file given."
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -s "p" -l "input-format" -d "[yaml|y|props|p|xml|x] parse format for input." -x
...
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -s "o" -l "output-format" -d "[yaml|y|json|j|props|p|xml|x] output format type." -x
...
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -s "s" -l "split-exp" -d "print each result (or doc) into a file named (exp)." -r
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -l "split-exp-file" -d "Use a file to specify the split-exp expression." -r
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -s "r" -l "unwrapScalar" -d "unwrap scalar, print the value with no quotes, colors or comments."
...
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -l "xml-attribute-prefix" -d "prefix for xml attributes (default \"+@\")" -x
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -l "xml-content-name" -d "name for xml content (if no attribute name is present)." -x
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -l "xml-directive-name" -d "name for xml directives (e.g. <!DOCTYPE thing cat>) (default \"+directive\")" -r
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -l "xml-keep-namespace" -d "enables keeping namespace after parsing attributes (default true)"
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -l "xml-proc-inst-prefix" -d "prefix for xml processing instructions (e.g. <?xml version=\"1\"?>) (default \"+p_\")" -x
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -l "xml-raw-token" -d "enables using RawToken method instead Token."
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -l "xml-skip-directives" -d "skip over directives (e.g. <!DOCTYPE thing cat>)"
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -l "xml-skip-proc-inst" -d "skip over process instructions (e.g. <?xml version=\"1\"?>)"
complete -c yq -n "not __fish_seen_subcommand_from eval eval-all help shell-completion" -l "xml-strict-mode" -d "enables strict parsing of XML."
